let app = {
    ...
    insertRowsNumbers() {
        ...
        for (let i = 0; i < trs.length; i++) {
            ...
            tdLast.innerHTML = this.config.rows[i];
            tdLast.style.transform = 'rotate(180deg)';
            trs[i].insertAdjacentElement("beforeend", tdLast);
        }
    },
    ...
    insertColsChars() {
        ...
        firstTr.innerHTML += `<td>`;
        for (let i = 0; i < this.config.cols.length; i++) {
            firstTr.innerHTML += `<td class='rotate'>${this.config.cols[i]}</td>`;
        }
        firstTr.innerHTML += `<td>`;
